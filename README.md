...
6. [Multiple Instances](#6-multiple-instances)
7. [S3-Backed Storage](#7-s3-backed-storage)
8. [Teardown](#8-teardown)
9. [Export Procedure](#9-export-procedure)
10. [Traefik Integration Notes](#10-traefik-integration-notes)
...
| `influxdb3_admin_token` | string | _(none)_ | Admin bearer token. Not used at deploy time — InfluxDB 3 Core does not accept a pre-set token via env var. Create post-deploy with `podman exec influxdb3-<instance> influxdb3 create token --admin`, then store with `ansible-vault`. |
